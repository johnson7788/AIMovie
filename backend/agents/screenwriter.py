...
class Screenwriter:
    ...
    async def develop_story(
        ...
        print(f"🧠 Developing story via {self.chat_model.model_name} ...")
        print(f"   idea: {idea[:200]}{'...' if len(idea) > 200 else ''}")
        if user_requirement:
            print(f"   requirement: {user_requirement[:200]}{'...' if len(user_requirement) > 200 else ''}")
        messages = [
        ...
        logger.info(f"Invoking {self.chat_model.model_name}, system_prompt_len={len(messages[0][1])}, human_prompt_len={len(messages[1][1])}")
        logger.info(f"Model config: model_name={self.chat_model.model_name}, openai_api_base={getattr(self.chat_model, 'openai_api_base', 'N/A')}, timeout={getattr(self.chat_model, 'request_timeout', 'N/A')}")
        t0 = __import__("time").time()
        response = await self.chat_model.ainvoke(messages)
        elapsed = __import__("time").time() - t0
        story = response.content
        print(f"✅ Story generation completed in {elapsed:.1f}s ({len(story)} chars)")
        return story
    ...
    async def write_script_based_on_story(
        ...
        print(f"📝 Writing script via {self.chat_model.model_name} ...")
        print(f"   story len: {len(story)} chars")
