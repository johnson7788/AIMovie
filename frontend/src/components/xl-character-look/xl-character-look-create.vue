...
const submit = () => {
    ...
    $http.post('/app/shortplay/api/CharacterLook/update', createForm).then((res: any) => {
        ...
        ElMessage.error(t('characterLook.createFail'));
    }).finally(() => {
        ...
        <el-dialog v-model="characterLookDialogVisible" class="generate-scene-dialog" draggable width="min(100%,800px)"
            ...
            <el-form label-position="right" label-width="80px" :model="createForm" size="large"
                ...
                <div class="grid-columns-2 grid-gap-x-10 grid-gap-y-4 character-look-form">
                    <el-form-item :label="t('characterLook.lookName')" prop="title" class="grid-column-2">
                        <el-input v-model="createForm.title" :placeholder="t('characterLook.lookNamePlaceholder')" class="form-input" />
                    </el-form-item>
                    <el-form-item :label="t('characterLook.overall')" prop="overall_style" class="grid-column-1">
                        <el-input v-model="createForm.overall_style" :placeholder="t('characterLook.overallPlaceholder')"
                            class="form-input" />
                    </el-form-item>
                    <el-form-item :label="t('characterLook.status')" prop="status_note" class="grid-column-1">
                        <el-input v-model="createForm.status_note" :placeholder="t('characterLook.statusPlaceholder')" class="form-input" />
                    </el-form-item>
                    <el-form-item :label="t('characterLook.makeup')" prop="makeup" class="grid-column-1">
                        <el-input v-model="createForm.makeup" :placeholder="t('characterLook.makeupPlaceholder')" class="form-input" />
                    </el-form-item>
                    <el-form-item :label="t('characterLook.hairStyle')" prop="hair_style" class="grid-column-1">
                        <el-input v-model="createForm.hair_style" :placeholder="t('characterLook.hairStylePlaceholder')" class="form-input" />
                    </el-form-item>
                    <div class="grid-column-2 flex flex-column grid-gap-4">
                        <span>{{ t('characterLook.costumeDesc') }}</span>
                        <div class="flex flex-column bg-overlay rounded-4 p-4">
                            <el-input type="textarea" v-model="createForm.costume" :placeholder="t('characterLook.costumeDescPlaceholder')"
                                class="form-textarea" :autosize="{ minRows: 3, maxRows: 6 }" />
                            <el-upload ref="uploadImageRef" :data="{ dir_name: 'actor/costume', dir_title: t('characterLook.costumeImage') }"
            ...
            <template #footer>
                <el-button @click="characterLookDialogVisible = false;" bg text :disabled="createLoading">{{ t('common.cancel') }}</el-button>
                ...
                <div class="flex flex-center grid-gap-2 input-button rounded-4 py-2 px-6" ref="actorCostumeButtonRef">
                    <template v-if="!actorCostumeModel.id">
                        <el-icon :alt="t('characterLook.characterCostume')" class="icon-model" color="var(--el-color-success)">
                            <IconModelSvg />
                        </el-icon>
                        <span class="h10">{{ t('characterLook.selectCostumeModel2') }}</span>
                    </template>
                    <template v-else>
                ...
                <el-button type="success" @click="submit()" :loading="createLoading">{{ t('common.create') }}</el-button>
            </template>
        </el-dialog>
